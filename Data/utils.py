...
def collate_fn_mmcpred(batch):
    """
    batch should be a list of (sequence, target, length) tuples...
    Returns a padded tensor of sequences sorted from longest to shortest,
    """

    all_input_ids, all_input_times = map(torch.stack, zip(*batch))
    return all_input_ids, all_input_times


def collate_fn_mmpred(batch):
    """
    batch should be a list of (sequence, target, length) tuples...
    Returns a padded tensor of sequences sorted from longest to shortest,
    """

    all_input_ids = map( torch.stack, zip(*batch))
