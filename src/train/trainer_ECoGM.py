...
relations = [
    "PersonX Need", "PersonX Intent", "PersonX Attr", "PersonX Effect",
    "PersonX React", "PersonX Want", "Other Effect", "Other React",
    "Other Want"
]
...
class Trainer(object):
    ...
    def __init__(self,
        ...
        self.start_epoch = 1
        self.global_step = 0
        self.model, self.device = model_device(n_gpu=args.n_gpu,
                                               model=self.model)
    ...
    def save_info(self, epoch, best):
        model_save = self.model.module if hasattr(self.model,
    ...
    def create_im_inputs(self,
        ...
        char_relation = self.tok(relations,
                                 padding=True,
                                 return_length=True,
                                 return_tensors='pt')
        ...
        for i in range(batch_size):
            ...
            for j in range(dim):
                ...
                if cur_ln + char_ln > ln:
                    ...
                    it = torch.cat([
                        cur_input_ids[:cur_ln], cur_relation_ids[:char_ln],
                        cur_input_ids[cur_ln + char_ln:]
                    ],
                                   dim=-1).unsqueeze(0)
                    cur_kg_input_ids.append(it)
            kg_input_ids.append(
                torch.cat(cur_kg_input_ids, dim=0).unsqueeze(0))
    ...
    def run_batch(self, batch, step, mode):
        ...
        batch_size, ln = input_ids.size()
        kg_input_ids, kg_input_masks = self.create_im_inputs(input_ids,
                                                             input_mask,
                                                             segment_ids,
                                                             event_only=True)
        ...
        logits_softmax = torch.where(
            logits_softmax < self.softmax_eps,
            torch.tensor(self.softmax_eps,
                         dtype=torch.float32).to(self.device), logits_softmax)
        ...
        explanation_kld_loss = torch.tensor([0], dtype=torch.float32).to(
            self.device)
        explanation_cls_loss = torch.tensor([0], dtype=torch.float32).to(
            self.device)
        ...
        if self.args.training_explanation:
            ...
            kg_decoder_input_ids = torch.full([kg_batch_size, 1],
                                              fill_value=self.tok.bos_token_id,
                                              dtype=torch.long).to(self.device)
            kg_decoder_input_mask = torch.ones_like(kg_decoder_input_ids,
                                                    dtype=torch.long).to(
                                                        self.device)

            kg_encoder_hidden_states = kg_hidden_states.reshape(
                [batch_size * dim, ln, -1]).to(self.device)
            kg_encoder_padding_mask = kg_input_masks.reshape(
                [batch_size * dim, ln]).to(self.device)
            kg_input_ids = kg_input_ids.reshape([batch_size * dim,
                                                 -1]).to(self.device)
            ...
            finished_sentences = torch.tensor([False] * (batch_size * dim)).to(
                self.device)

            for i in range(explanation_len):
                decoder_outputs = kg_decoder(
                    inputs_embeds=kg_decoder_word_emb,
                ...
                vocab_logits = kg_model.lm_head(
                    decoder_outputs[0]) + kg_model.final_logits_bias

                vocab_logits = vocab_logits[:, -1, :]

                if self.args.use_kld_loss:
                    lm_outputs = kg_model(
                        input_ids=kg_input_ids,
                    ...
                    explanation_kld_loss += kld_loss(vocab_logprobs,
                                                     lm_vocab_probs)

                oh_vocab_probs = F.gumbel_softmax(
                    vocab_logits, hard=False).unsqueeze(1)  # output soft probs
                chosen_emb = torch.matmul(
                    oh_vocab_probs, kg_embeddings_weight.weight)  # [B, E]

                max_prob_idx = torch.argmax(oh_vocab_probs, dim=-1).squeeze()
                # [B] predicted next word according to the gumbel-softmax probs

                next_word = max_prob_idx.masked_fill(
                    finished_sentences.eq(True), self.tok.pad_token_id)
                finished_sentences = finished_sentences.masked_fill(
                    max_prob_idx.eq(self.tok.eos_token_id), True)

                kg_decoder_input_ids = torch.cat(
                    [kg_decoder_input_ids,
                     next_word.unsqueeze(1)], dim=-1)
                kg_decoder_word_emb = torch.cat(
                    (kg_decoder_word_emb, chosen_emb), dim=1)
                kg_decoder_input_mask = kg_decoder_input_ids.ne(
                    self.tok.pad_token_id).int().to(self.device)
            ...
            if self.args.use_cls_loss:
                ...
                eos_mask = kg_decoder_input_ids[:, :-1].eq(
                ...
                eos_hidden_states = kg_decoder_hidden_states[eos_mask, :].view(
                    kg_decoder_hidden_states.size(0), -1,
                    kg_decoder_hidden_states.size(-1))[:, -1, :]  # [bsz, E]
                lm_cls_logits = kg_model.classification_head(
                    eos_hidden_states)  # [bsz, num_labels]
                explanation_cls_label = torch.zeros([lm_cls_logits.size(0)],
                                                    dtype=torch.long).to(
                                                        self.device)
                explanation_cls_loss += cross_entropy_loss(
                    lm_cls_logits, explanation_cls_label)
        ...
        self.epoch_loss_agg.update(
            self.args.explanation_coeff * loss.item() +
            (explanation_kld_loss.item() + explanation_cls_loss.item()),
            n=batch_size)

        # print(f"loss: {loss.size()}, "
        #       f"explanation_kld_loss: {explanation_kld_loss.size()},"
        #       f"explanation_cls_loss: {explanation_cls_loss.size()}")
        loss = self.args.explanation_coeff * loss + (explanation_kld_loss +
                                                     explanation_cls_loss)
    ...
    def model_fit(self, data, mode, print_interval, pbar):
        for step, batch in enumerate(data):
            ...
            if output_ids.size(0):
                batch_pred_strs = self.tok.batch_decode(
                    output_ids, skip_special_tokens=True)
                batch_target_strs = self.tok.batch_decode(
                    target_ids, skip_special_tokens=True)

                if self.batch_metrics:
                    for metric in self.batch_metrics:
                        if metric.name() == "Rouge":
                            metric(pred_str=batch_pred_strs,
                                   label_str=batch_target_strs)
                            self.info[metric.name()] = metric.value()
                        elif metric.name() == "accuracy":
                            metric(logits=logits.contiguous().view(
                                -1, logits.shape[-1]),
    ...
    def run_epoch(self, data, mode, filename=None):
        ...
        if mode == "train":
            ...
            if self.epoch_metrics:
                for metric in self.epoch_metrics:
                    if metric.name() == "Rouge":
                        metric(pred_str=self.pred_strs,
                               label_str=self.label_strs)
                    else:
    ...
    def train(self, train_data, valid_data):
        ...
        seed_everything(
            self.args.seed
        )  # Added here for reproductibility (even between python 2 a
        for epoch in range(self.start_epoch,
            ...
            if self.model_checkpoint:
                ...
                state = self.save_info(
                    epoch, best=logs[self.model_checkpoint.monitor])
                self.model_checkpoint.epoch_step(
...
class TrainerCls(object):
    ...
    def run_batch(self, batch, step, mode):
        ...
        if label_indice.size(0) != 0:
            ...
            if self.args.training_explanation:
                ...
                kg_hidden_states = outputs[
                    "kg_hidden_states"]  # [bsz, dim, ln, E]
                kg_hidden_states = kg_hidden_states.index_select(
                    dim=0, index=label_indice)
                kg_input_ids = kg_input_ids.index_select(dim=0,
                                                         index=label_indice)
                kg_input_masks = kg_input_masks.index_select(
                    dim=0, index=label_indice)

                true_data_bsz, dim, ln = kg_input_masks.size()

                kg_decoder_input_ids = torch.ones(
                    [true_data_bsz * dim, 1], dtype=torch.long).to(
                ...
                kg_encoder_hidden_states = kg_hidden_states.reshape(
                    [true_data_bsz * dim, ln, -1]).to(self.device)
                kg_encoder_padding_mask = kg_input_masks.reshape(
                    [true_data_bsz * dim, ln]).to(self.device)
                kg_input_ids = kg_input_ids.reshape([true_data_bsz * dim,
                                                     -1]).to(self.device)
                ...
                kg_decoder = kg_model.model.get_decoder()
                kg_embeddings_weight = kg_model.model.get_input_embeddings()
                kg_decoder_word_emb = kg_embeddings_weight(
                    kg_decoder_input_ids)

                finished_sentences = torch.tensor(
                    [False] * (true_data_bsz * dim)).to(self.device)
                ...
                for i in range(explanation_len):
                    ...
                    if self.args.use_kld_loss:
                        ...
                        lm_vocab_probs = torch.softmax(lm_vocab_logits, dim=-1)
                        vocab_logprobs = torch.log_softmax(vocab_logits,
                                                           dim=-1)
                        explanation_kld_loss += kld_loss(
                            vocab_logprobs, lm_vocab_probs)

                    oh_vocab_probs = F.gumbel_softmax(
                        vocab_logits,
                        hard=False).unsqueeze(1)  # output soft probs
                    chosen_emb = torch.matmul(
                        oh_vocab_probs, kg_embeddings_weight.weight)  # [B, E]

                    max_prob_idx = torch.argmax(oh_vocab_probs,
                                                dim=-1).squeeze()
                    # [B] predicted next word according to the gumbel-softmax probs
                ...
                if self.args.use_cls_loss:
                    ...
                    eos_hidden_states = kg_decoder_hidden_states[
                        eos_mask, :].view(kg_decoder_hidden_states.size(0), -1,
                                          kg_decoder_hidden_states.size(
                                              -1))[:, -1, :]  # [bsz, E]
                    lm_cls_logits = kg_model.classification_head(
                        eos_hidden_states)  # [bsz, num_labels]
                    explanation_cls_label = torch.zeros(
                        [lm_cls_logits.size(0)],
                        dtype=torch.long).to(self.device)
                    explanation_cls_loss += cross_entropy_loss(
                        lm_cls_logits, explanation_cls_label)
            ...
            if self.args.training_explanation:
                if self.args.use_kld_loss:
                    self.info[
                        f"explanation-kld-loss"] = explanation_kld_loss.item()
                if self.args.use_cls_loss:
                    self.info[
                        f"explanation-cls-loss"] = explanation_cls_loss.item()

            self.epoch_loss_agg.update(
                loss.item() * self.args.explanation_coeff +
                (explanation_kld_loss.item() + explanation_cls_loss.item()),
    ...
    def model_fit(self, data, mode, print_interval, pbar):
        for step, batch in enumerate(data):
            ...
            cls_accuracy(logits=cls_logits.contiguous().view(
                -1, cls_logits.shape[-1]),
