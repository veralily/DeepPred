...
class Precision(Metric):
    ...
    def __call__(self, prediction, target):
        """
        :param prediction: Tensor [num, ln]
        :param target: Tesnor [num, ln]
        :return:
        """
        correct = prediction.eq(target)
        self.correct_k = correct.contiguous().view(-1).float().sum(0)
        self.total = target.view(-1).size(0)
...
class MAE(Metric):
    ...
    def __call__(self, prediction, target):
        ...
        self.AE = self.diff.abs()

    def reset(self):
        self.AE = None

    def value(self):
        return float(self.AE.mean())
...
def calculate_bleu(output_lns, refs_lns, ngram=2, **kwargs) -> dict:
    ...
    references: [['this', 'is', 'a', 'inference], ['this', 'is', 'inference', 'too']]
    hypothesis: ['a', 'inference', 'here'],
