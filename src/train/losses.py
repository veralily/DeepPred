...
def sequence_loss(logits, target):
    ...
    return loss(input=logits, target=target)


def huber_loss(logits, target, beta: float = 1.0):
    """
    :param beta: threshold
    :param logits: any shape
    :param target: the same as logits
    :return:
    """
    loss = nn.SmoothL1Loss(reduction='mean', beta=beta)
    return loss(input=logits, target=target)