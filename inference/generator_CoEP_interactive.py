...
class Generator(object):
    ...
    def generate_example(self,
        ...
        with torch.no_grad():
            for step, batch in enumerate(data):
                ...
                relation_mask = char_relation["attention_mask"].to(
                    input_ids.device)
                ...
                decoder_input_ids = torch.ones(
                    [batch_size, 1], dtype=torch.long) * self.tok.bos_token_id
                decoder_input_ids = decoder_input_ids.to(self.device)
                batch_data = {}
                batch_data["input_ids"] = decoder_input_ids
                batch_data["input_attention_mask"] = torch.ones_like(
                    decoder_input_ids, dtype=torch.long)
                ...
                input_sent = self.tok.batch_decode(input_ids,
    ...
    def interactive_generate_example(self,
        ...
        with torch.no_grad():
            ...
            kg_input_ids = torch.cat(cur_kg_input_ids,
                                     dim=0).unsqueeze(0).to(self.device)
            kg_input_masks = kg_input_ids.ne(self.tok.pad_token_id).int().to(
                self.device)
            ...
            decoder_input_ids = torch.ones(
                [1, 1], dtype=torch.long) * self.tok.bos_token_id
            decoder_input_ids = decoder_input_ids.to(self.device)
            batch_data = {}
            batch_data["input_ids"] = decoder_input_ids
            batch_data["input_attention_mask"] = torch.ones_like(
    ...
    def interactive_generate_explanation(self,
        ...
        with torch.no_grad():
            ...
            encoder_outputs = encoder(
                input_ids=kg_input_ids.reshape([dim, ln]),
                attention_mask=kg_input_masks.reshape([dim, ln]),
                inputs_embeds=None,
            ...
            decoder_input_ids = torch.ones(
                [dim, 1], dtype=torch.long) * self.tok.bos_token_id
            decoder_input_ids = decoder_input_ids.to(self.device)
            batch_data = {}
            batch_data["input_ids"] = decoder_input_ids
            batch_data["input_attention_mask"] = torch.ones_like(
                decoder_input_ids, dtype=torch.long).to(self.device)
            batch_data["encoder_hidden_states"] = KG_hidden_states.reshape(
                [dim, ln, -1]).to(self.device)
            batch_data["encoder_padding_mask"] = kg_input_masks.reshape(
                [dim, ln]).to(self.device)
            ...
            for j in range(int(dim / num_return_sequences)):
                seqs_list.append('|'.join(
                    seqs[j * num_return_sequences:(j + 1) *
